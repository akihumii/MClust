...
if isempty(MClust_CutOnBestProj)
    for iF = 1:length(MClust_FeatureNames) - 1
        ...
        if isempty(strmatch(FeatureToGet(1:(FindColon - 1)),MClust_CutOnBestProj,'exact'))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function CBP_Callbacks(cboHandle)
...
global MClust_CutOnBestProj CBP_data
global MClust_FeatureTimestamps MClust_FeatureData MClust_FDfn MClust_Clusters
global MClust_Clusters MClust_FeatureData MClust_TTdn MClust_TTfn MClust_TText MClust_ChannelValidity
...
switch SwitchTag
    ...
    case 'CalcBestProj'
        ...
        [f1 MClust_Clusters{iClust}] = FindInCluster(MClust_Clusters{iClust}, MClust_FeatureData);

        if iClust2 ~= 0
            [f2 MClust_Clusters{iClust2}] = FindInCluster(MClust_Clusters{iClust2}, MClust_FeatureData);
        else
            f2 = find(~ismember(1:size(MClust_FeatureData,1),f1))';
            set(findobj(gcf,'TooltipString', 'Add limit to Cluster B'),'enable','off')
            set(findobj(gcf,'Tag', 'Swap'),'enable','off')
    ...
    case {'Cluster1pulldownCBP';'Cluster2pulldownCBP'}
        ...
        if newClust1
            if ~isempty(strmatch('keeps1',fields(CBP_data)))
                ...
                [f1 MClust_Clusters{iClust1}] = FindInCluster(MClust_Clusters{iClust1}, MClust_FeatureData);
        ...
        if newClust2
            if ~isempty(strmatch('keeps1',fields(CBP_data)))
                ...
                if iClust2 ~= 0 % added to stop error if user sets cluster B back to zero - JCJ Sept 2007
                    ...
                    f2 = find(~ismember(1:size(MClust_FeatureData,1),CBP_data.f1))';
                    set(findobj(gcf,'TooltipString', 'Add limit to Cluster B'),'enable','off')
                    set(findobj(gcf,'Tag', 'Swap'),'enable','off')
...
global CBP_data MClust_TTData MClust_Clusters MClust_FeatureData MClust_FeatureTimestamps

f1=FindInCluster(MClust_Clusters{CBP_data.iClust1},MClust_FeatureData);
if CBP_data.iClust2
    f2=FindInCluster(MClust_Clusters{CBP_data.iClust2},MClust_FeatureData);
end

f1n=FindInCluster(Restrict_Points(MClust_Clusters{CBP_data.iClust1},CBP_data.f1(find(~CBP_data.keeps1))),MClust_FeatureData);
if CBP_data.iClust2
    f2n=FindInCluster(Restrict_Points(MClust_Clusters{CBP_data.iClust2},CBP_data.f2(find(~CBP_data.keeps2))),MClust_FeatureData);
end
...
global CBP_data
global MClust_Colors MClust_Clusters MClust_Temp_FeatureNames
...
global MClust_Temp_FeatureNames MClust_Clusters MClust_FeatureData MClust_FDfn
...
FD_temp = repmat(nan,size(MClust_FeatureData,1),2);
