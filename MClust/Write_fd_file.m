...
% 		[block_t,wv] = MClust_LoadNeuralData(TT_file_name,[starts(block) ends(block)],4);  % ORIGNAL CODE TO LOAD FILE
        % --------------------------------------------------------
        % Coded by Tsai Chne Wuen 20190925
        if strfind(TT_file_name, '.mat')
            dataoutput = load(TT_file_name);  % load the structure containing wv and block_t from .mat file and assign it into 'dataoutput'
            wv = dataoutput.wv;
            block_t = dataoutput.block_t;
        else
